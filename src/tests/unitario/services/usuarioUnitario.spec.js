...
describe("usuarioService", () => {
  ...
  describe("listar", () => {
    ...
    it("Deve listar dados de usuários com filtro  de pagina e quantidade como números strings", async () => {
      const usuarioMock = [
        { id: 1, nome: "John", email: "john@example.com", senha: "secret" },
      ];
      UsuarioRepository.findMany.mockResolvedValue(usuarioMock);
      ...
      const filtro = { id: 1, pagina:'1', quantidade:'10' };
      const result = await usuarioService.listar(filtro);
      expect(result).toHaveProperty("data");
      expect(result.data).toBeDefined();
      expect(result.data).toBeInstanceOf(Array);
    ...
    it("Deve listar dados de usuários com filtro de pagina e quantidade errados", async () => {
      const usuarioMock = [
        { id: 1, nome: "John", email: "john@example.com", senha: "secret" },
      ];
      UsuarioRepository.findMany.mockResolvedValue(usuarioMock);
      ...
      const filtro = { id: 1, pagina:'a', quantidade:'b' };
      const result = await usuarioService.listar(filtro);
      expect(result).toHaveProperty("data");
      expect(result.data).toBeDefined();
      expect(result.data).toBeInstanceOf(Array);
  ...
  describe("inserir", () => {
    describe("Sucesso", () => {
      ...
      it("Deve ter sucesso quando cadastrar um usuario passando um mock null para o repository", async () => {
        const data = {
          nome: "John",
          email: "john@example.com",
          senha: "StrongPass1!",
